...
async function bridge(): Promise<void> {
  ...
  // Parse quotes:
  //   quotes[0] = { token: address(0), amount: gasPayment }  (native)
  //   quotes[1] = { token: transferTokenAddr, amount: ... }   (transfer token)
  //   quotes[2] = { token: feeTokenAddr, amount: fee }        (fee token, optional)
  const gasPayment = quotes[0].amount;
  const transferTokenAddr = quotes[1].token;
  const transferTokenAmount = quotes[1].amount;
